...
(function(){
  ...
  const monthOf=v=>{
    if(!v)return '';
    const raw=String(v);
    const direct=raw.match(/(20\d{2})-(\d{2})/);
  ...
  const monthList=(approved)=>{
    const set=new Set(approved.map(o=>monthOf(o.created_at)).filter(Boolean));
    const now=new Date();
    // Always expose every month of the current year up to the current month,
    // then append any older months that actually exist in the sales history.
    for(let m=1;m<=now.getMonth()+1;m++) set.add(`${now.getFullYear()}-${pad(m)}`);
  ...
  window.salesPage=async function(){
    try{
      ...
      const months=monthList(approved);
      let selected=window.cassanoSalesMonth||nowMonth();
      if(selected!=='all' && !months.includes(selected)) selected=nowMonth();
