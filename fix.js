...
(function(){
  async function readResponse(r){
    ...
    if(!r.ok) throw new Error(data.error||('Gagal menyimpan (HTTP '+r.status+')'));
    return data;
  }
  function compressImage(file){
    return new Promise((resolve,reject)=>{
      ...
      reader.onload=()=>{img.onload=()=>{
        ...
        c.toBlob(b=>b?resolve(new File([b],'item-photo.jpg',{type:'image/jpeg'})):reject(new Error('Foto gagal diproses')),'image/jpeg',.82);
      };img.onerror=()=>reject(new Error('Foto tidak bisa dibaca'));img.src=reader.result};
      reader.onerror=()=>reject(new Error('Foto tidak bisa dibaca'));reader.readAsDataURL(file);
    });
  }
  window.itemModal=async function(id){
    ...
    openM(i?'Edit Barang':'Tambah Barang',`<form id="f" enctype="multipart/form-data">
      ...
      <input id="stock" type="number" min="0" step="1" placeholder="Stok" value="${i?.stock||0}">
    ...
    $('f').onsubmit=async e=>{
      e.preventDefault();
      const btn=$('saveItem');btn.disabled=true;btn.textContent='Menyimpan...';
      try{
        const fd=new FormData();
        fd.append('code',$('code').value.trim());fd.append('name',$('name').value.trim());fd.append('category_id',$('cat').value);fd.append('price',$('price').value||0);fd.append('stock',$('stock').value||0);fd.append('description',$('desc').value);
        const file=$('photo').files[0]; if(file) fd.append('photo',await compressImage(file));
        const r=await fetch('/api/items'+(id?'/'+id:''),{method:id?'PUT':'POST',body:fd,credentials:'same-origin'});
        await readResponse(r);
        closeM();await refresh();toast(i?'Perubahan barang berhasil disimpan':'Barang berhasil ditambahkan');
      }catch(err){toast(err.message||'Gagal menyimpan barang');console.error(err)}
